...
do
    ...
    /usr/bin/psql -U pggis -h localhost -w -c "CREATE EXTENSION postgis; CREATE EXTENSION postgis_topology; drop type if exists texture; create type texture as (url text,uv float[][]);" $DBNAME
...
/usr/bin/psql -U pggis -h localhost -w -c "CREATE EXTENSION postgis; CREATE EXTENSION postgis_topology;  drop type if exists texture;
